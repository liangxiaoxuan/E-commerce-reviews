...
```
#We extracted unique number of each column. We have 1206 unique clothing ID,  77 different Age levels, 5 levels of rating from 1 to 5; and so on
...
#The variable recommended.IND is binary, and we can see that most people would like to recommend item they have purchased to other people.However, from positive.feedback.count histogram plot, we can see that a large amount of people did not leave positive feedback continuously. We assumed that there were three possibilities to cause this outcome. First,  people were not satisfied with their shopping experience. For example, they picked a wrong size according to size information.  Second, some of them were new of this e-commerce website. Third, they just were too lazy to do it.
...
```
#We also did a histogram for clothing ID and extracted the Id1078 which has the largest amount. We can see that clothing id 1078 belongs to dresses class. We also want to analyze people’s purchase ratio of different ages. 30-40 years of age accounted for the highest amount of purchasing product Id1078, and 40-50 years of age accounted for the second highest amount.
...
```
# We picked 18 words. Among those words, all of them are positive sentiments and "love" has the largest amount.
...
```
#We subdivided all rating into three different levels: 4-5 as high rate; 3 as medium rate; 1-2 as low rate. Based on these three datasets, we did word frequency of title and class name frequency respectively.
#From the high rate plot, we can conclude that all words selected are positive sentiment, customers were satisfied with the product they purchased. From the medium rate plot, we can see that some negative words started existing. Those words were more foucusing on design of the clothing. From the low rate plot, there were more negative sentiment words existing than medium rate. Those negative words were more likely disappointed about the quality of the cloth rather than design of the cloth.
...
```
#We did class.name frequency of each different rating level. Based on the results, we can conclude that Blouses, Dresses, and Knits are bestseller of this e-commerce website. However, since there still were some medium and low rating of Blouses, Dresses, and Knits, this e-commerce website should figure out how to improve the rating of these two rating levels.
